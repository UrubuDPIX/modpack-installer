    ...
    try {
        ...
        if (provider === 'modrinth') {
            ...
            if (version_id !== 'latest') {
                const specificVer = versions.find((v) => v.id === version_id || v.name === version_id);
                if (specificVer)
                    ver = specificVer;
            }
            ...
            if (fileRes.ok) {
                ...
                const rawLoader = file.sortableGameVersions?.find((v) => ['Forge', 'Fabric', 'NeoForge', 'Quilt'].includes(v.gameVersionName))?.gameVersionName || 'Forge';
        ...
        let modpack = await index_1.prisma.modpack.findFirst({
            where: { name: modpackName }
        });
        if (!modpack) {
            modpack = await index_1.prisma.modpack.create({
                data: {
                    name: modpackName,
                    slug: modpackSlug,
            ...
            modpack = await index_1.prisma.modpack.update({
                where: { id: modpack.id },
                data: {
                    description: modpackDescription,
                    icon: modpackIcon,
        ...
        let modpackVersion = await index_1.prisma.modpackVersion.findFirst({
            where: {
        ...
        if (!modpackVersion) {
            modpackVersion = await index_1.prisma.modpackVersion.create({
                data: {
                    modpack_id: modpack.id,
                    version: versionName,
            ...
            modpackVersion = await index_1.prisma.modpackVersion.update({
        ...
        const result = await (0, installer_1.installModpack)(serverId, String(modpack.id), String(modpackVersion.id), 'install', shouldDelete);
...
router.delete('/:id/modpack/uninstall', async (req, res) => {
    try {
        const serverId = req.params.id;
        await (0, installer_1.uninstallModpack)(serverId);
...
router.get('/:id/modpack/metadata', async (req, res) => {
    try {
        ...
        try {
            ...
            if (serverModpack) {
                const dbModpack = await index_1.prisma.modpack.findUnique({
                    where: { id: serverModpack.modpack_id }
                });
...
router.get('/:id/modpack/versions', async (req, res) => {
    try {
        ...
        if (!modpack)
            return res.status(404).json({ message: 'Modpack não encontrado no banco' });
        ...
        if ((modpack.source || '').toLowerCase() === 'modrinth') {
            ...
            if (!resp.ok)
                return res.status(400).json({ message: 'Erro Modrinth' });
            ...
            const cfKey = await getCurseForgeKey();
            if (!cfKey)
                return res.status(400).json({ message: 'Chave CurseForge não configurada no painel' });
            ...
            const data = await resp.json();
            return res.json(data.data.map((f) => ({
                id: String(f.id),
...
//# sourceMappingURL=serverModpacks.js.map