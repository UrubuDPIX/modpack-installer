...
import { faDownload, faBox, faCheck, faExclamationTriangle, faSearch, faFilter } from '@fortawesome/free-solid-svg-icons';
...
const MINECRAFT_VERSIONS = ['1.21.4', '1.21.1', '1.20.6', '1.20.4', '1.20.1', '1.19.4', '1.19.2', '1.18.2'];
...
export default function ModpacksContainer({ serverId }: ModpacksContainerProps) {
    ...
    return (
        <div className="p-6" style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <p className="text-gray-400 text-sm mb-6">Descubra e instale modpacks populares do Modrinth no seu servidor</p>
            ...
            {installedModpack && (
                <div className="bg-green-900/30 border border-green-700 p-3 rounded mb-4">
                    ...
                    <p className="text-gray-300 text-xs">Versão: {installedModpack.version?.version} | Status: {installedModpack.server_modpack?.status}</p>
                </div>
            )}
                        ...
                        >
                            ...
                            <div className="flex flex-wrap gap-1.5 mb-3">
                                {modpack.categories.slice(0, 3).map(cat => (
                                    <span key={cat} className="bg-gray-700/50 text-gray-300 px-2 py-0.5 rounded text-[10px] uppercase tracking-wide">
                                        {cat}
                                    </span>
                                ))}
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-3 border-t border-gray-700/50">
                                <div className="flex flex-col gap-0.5">
                                    {modpack.game_versions.slice(0, 1).map(v => (
                                        <span key={v} className="text-[10px] text-gray-500">MC {v}</span>
                                    ))}
                                    {modpack.loaders.slice(0, 1).map(l => (
                                        <span key={l} className="text-[10px] text-gray-500 capitalize">{l}</span>
                                    ))}
                                </div>
                                <button
