...
export default function ModpacksContainer() {
    ...
    fetch(`/api/client/servers/${id}/modpack/versions`)
      ...
      .then(versions => {
        if (versions && versions.length > 0) {
          ...
          const installedVer = extractVer(installedModpack.version);
          
          let hasUpdate = false;
          // If we couldn't extract a version, fallback to exact match. Otherwise compare extracted versions.
          if (latestVer && installedVer && latestVer !== installedVer) {
            hasUpdate = true;
          } else if (!latestVer && latest.name !== installedModpack.version) {
             hasUpdate = true;
          }
  ...
  const openUpdateModal = async () => {
    ...
    try {
      if (!id) throw new Error("ID do servidor não encontrado");
      const res = await fetch(`/api/client/servers/${id}/modpack/versions`);
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(errData.message || `Erro da API: ${res.status}`);
      }
      const data = await res.json();
      setUpdateVersions(data);
    } catch (e: any) {
      console.error(e);
