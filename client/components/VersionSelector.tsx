...
interface VersionSelectorProps {
  ...
  onSelect: (modpack: Modpack, version: ModpackVersion, action: 'install' | 'reinstall' | 'downgrade' | 'update') => void;
}

export default function VersionSelector({ modpack, currentVersionId, onSelect }: VersionSelectorProps) {
  const [isExpanded, setIsExpanded] = useState(true);

  const getActionType = (versionId: string): 'reinstall' | 'update' | 'downgrade' | 'install' => {
    if (!currentVersionId) return 'install';
    if (versionId === currentVersionId) return 'reinstall';
    
    const currentIndex = modpack.versions.findIndex(v => v.id === currentVersionId);
  ...
  return (
    <div className="version-selector">
      <div className="version-header" onClick={() => setIsExpanded(!isExpanded)}>
        <img src={modpack.icon || '/default-modpack.png'} alt={modpack.name} className="version-modpack-icon" />
              ...
              >
                <div className="version-info-row">
                  <div className="version-main">
                    ...
                    {version.isServerPack && <span className="server-pack-badge">Server Pack ✓</span>}
                  </div>
                  
                  <div className="version-meta">
                    <span className="mc-version">{version.minecraftVersion}</span>
                    <span className="loader-type">{version.loader} {version.loaderVersion}</span>
                    <span className="file-size">{version.size}</span>
                    <span className="release-date">{new Date(version.releasedAt).toLocaleDateString('pt-BR')}</span>
                  </div>

                  <div className="version-actions">
                    <span className={`release-tag ${version.isRelease ? 'release' : 'beta'}`}>
